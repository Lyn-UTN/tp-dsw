...
export function sanitizeVehiculoInput(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  Object.keys(sanitizedInput).forEach((key) => {
    if (
      sanitizedInput[key] === undefined ||
      sanitizedInput[key] === "" ||
      isNaN(sanitizedInput[key])
    ) {
...
export async function findOne(req: Request, res: Response) {
  try {
    const vehiculo = await em.findOne(Vehiculo, {
      idVehiculo: Number(req.params.id),
    });
    if (!vehiculo)
...
export async function updateVehiculo(req: Request, res: Response) {
  try {
    const vehiculoToUpdate = await em.findOne(Vehiculo, {
      idVehiculo: Number(req.params.id),
    });
    if (!vehiculoToUpdate)
      return res.status(404).json({ message: "Vehiculo no encontrado" });
