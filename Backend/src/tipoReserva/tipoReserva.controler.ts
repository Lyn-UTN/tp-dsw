...
function sanitizeTipoReservaInput(req: Request, res: Response, next: NextFunction) {
...
async function findAll(req: Request, res: Response) {
  try {
    ...
    res.status(200).json({ message: "Todos los tiposReserva", data: tiposReserva })
  } catch (error: any) {
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    const tipoReserva = await em.findOneOrFail(Tiporeserva, { idtiporeserva: id })
    res.status(200).json({ message: "Un tipoReserva especifico", data: tipoReserva })
  } catch (error: any) {
    res.status(500).json({ message: error.message })
  }
}
...
async function update(req: Request, res: Response) {
  try {
    const id = Number.parseInt(req.params.id)
    const tipoReservaToUpdate = await em.findOneOrFail(Tiporeserva, { idtiporeserva: id })
    em.assign(tipoReservaToUpdate, req.body.sanitizedInput)
    await em.flush()
    res
      .status(200)
      .json({ message: "tipoReserva modificado/update", data: tipoReservaToUpdate })
  } catch (error: any) {
    res.status(500).json({ message: error.message })
  }
}
