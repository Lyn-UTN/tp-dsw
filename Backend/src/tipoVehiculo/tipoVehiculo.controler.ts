...
async function findAll(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({ message: "found all tipoVehiculos", data: tipoVehiculos });
  } catch (error: any) {
...
async function add(req: Request, res: Response) {
  try {
    ...
    res
      .status(201)
      .json({ message: "tipoVehiculo created", data: tipoVehiculo });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
}
...
async function update(req: Request, res: Response) {
  try {
    const tipoVehiculoToUpdate = await em.findOneOrFail(TipoVehiculo, {
      idTipoVehiculo: Number(req.params.id),
    });
...
async function remove(req: Request, res: Response) {
  try {
    const tipoVehiculo = em.findOne(TipoVehiculo, {
      idTipoVehiculo: Number(req.params.id),
    });
    if (!tipoVehiculo)
      return res.status(404).json({ message: "TipoVehiculo no encontrado" });
